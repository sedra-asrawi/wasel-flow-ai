...
serve(async (req) => {
  ...
  try {
    const { driverId = 1, orderType = "pickup" } = await req.json().catch(() => ({}));
    
    // Create test QR data that matches what our AI verification expects
    const qrData = `DRIVER_ID:${driverId}|ORDER_ID:ORD-2024-001|TYPE:${orderType}|CUSTOMER:John Smith|ADDRESS:123 Main St`;
